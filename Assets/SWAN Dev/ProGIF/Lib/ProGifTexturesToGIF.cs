...
public class ProGifTexturesToGIF : MonoBehaviour
{
    ...
    public string Save(List<Texture2D> textureList, int width, int height, int fps, int loop, int quality, string savePath,
        ...
        ResolutionHandle resolutionHandle = ResolutionHandle.ResizeKeepRatio, bool autoClear = true, bool destroyOriginTexture = false, bool smooth_yieldPerFrame = false)
    {
        ...
        return _Save(textureList, width, height, fps, loop, quality, savePath, onFileSaved, onFileSaveProgress, autoClear, destroyOriginTexture, smooth_yieldPerFrame);
    }
    ...
    /// <param name="destroyOriginTexture">If set to <c>true</c>, clear the original textures after processed.</param>
    /// <param name="smooth_yieldPerFrame">Process each texture in separate frame to avoid blocking the main thread, so the UI smoother.</param>
    public string Save(List<Texture2D> textureList, int width, int height, float frameDelay, int loop, int quality, string savePath,
        Action<int, string> onFileSaved = null, Action<int, float> onFileSaveProgress = null,
        ResolutionHandle resolutionHandle = ResolutionHandle.ResizeKeepRatio, bool autoClear = true, bool destroyOriginTexture = false, bool smooth_yieldPerFrame = false)
    {
        frameDelay_Override = frameDelay;
        this.m_ResolutionHandle = resolutionHandle;
        return _Save(textureList, width, height, 0, loop, quality, savePath, onFileSaved, onFileSaveProgress, autoClear, destroyOriginTexture, smooth_yieldPerFrame);
    }

    private string _Save(List<Texture2D> textureList, int width, int height, int fps, int loop, int quality, string savePath,
